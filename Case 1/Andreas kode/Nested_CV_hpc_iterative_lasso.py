...
if __name__ == "__main__":
    ...
    # Model parameters
    alphas_lasso = np.linspace(0.1, 3, 30)

    # Imputation parameters
    imputer_estimators = [(BayesianRidge(), 100), (BayesianRidge(), 115)]

    RMSE = {"Lasso": np.zeros(K_outer)}
    ...
    for k1, (train_index, test_index) in enumerate(cv_outer.split(X)):
        ...
        model_dict_errors = {"Lasso": errors_lasso}

        for model, errors in model_dict_errors.items():
            mean_error = errors.mean(axis=0)
            idx = np.unravel_index(np.argmin(mean_error, axis=None), mean_error.shape)
            imputer = IterativeImputer(
                estimator=imputer_estimators[idx[0]][0],
                n_nearest_features=imputer_estimators[idx[0]][1],
                max_iter=100,
                random_state=42,
            ).fit(all_data)
            ...
            best_inner = Lasso(alpha=alphas_lasso[idx[1]], max_iter=10000).fit(
    ...
    print(
        "Lasso mean RMSE:",
        round(RMSE["Lasso"].mean(), 4),
        ", std:",
        round(RMSE["Lasso"].std(), 4),
    )
